...
public class CollectionFactory : BuiltInComponentBase
{
	...
	/// <summary>
	///     This decreases the reference count for each resource loaded with collectionfactory.load. If reference is zero, the
	...
	/// <summary>
	///     Resources loaded are referenced by the collection factory component until the existing (parent) collection is
	///     destroyed or collectionfactory.unload is called.
	///     Calling this function when the factory is not marked as dynamic loading does nothing.
	...
	///     Spawning is instant, but spawned game objects get their first update calls the following frame. The supplied
	///     parameters for position, rotation and scale
	...
	///     <span class="icon-attention"></span> Calling
	///     <a href="/ref/collectionfactory#collectionfactory.create">collectionfactory.create</a> create on a collection
	///     factory that is marked as dynamic without having loaded resources
	///     using <a href="/ref/collectionfactory#collectionfactory.load">collectionfactory.load</a> will synchronously load
	///     and create resources which may affect application performance.
